...
def generate_particles(pos):
    ...
    In this file, Nearby_flag is set to True to solve a local localization problem. 
    And after the particle correctly converges to the actual position of the robot, 
    drag the robot in CoppeliaSim with the mouse to simulate the kidnapping (move at least 2m), 
    show that the particles can converge to the position after kidnapping. 
    ...
    if Nearby_flag:
        x = pos[0] - 0.2 + 0.4 * np.random.random(size=(1, NP))
        y = pos[1] - 0.2 + 0.4 * np.random.random(size=(1, NP))
...
def re_sampling(px, pw):
    ...
    random_indices = np.random.choice(indices, max(
        0, (int)(NP*(1.0-w_fast/w_slow))))
    proposol_particles = np.array(
        [[px[j][pw_indices[i]] + (np.random.rand()-0.5)/5 for i in range(NP)] for j in range(3)])
    print(w_fast, w_slow, len(random_indices))
    for idx in random_indices:
        proposol_particles[0][idx] = np.random.uniform(
            0, 5)
        proposol_particles[1][idx] = np.random.uniform(
            0, 5)
        proposol_particles[2][idx] = np.random.uniform(0, math.pi)
    ###  END CODE HERE  ###
